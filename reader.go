...
func (r *Reader) run(cg *ConsumerGroup) {
	...
	for {
		...
		for attempt := 1; attempt <= r.config.MaxAttempts; attempt++ {
			// 一旦cg.Next返回了gen，那么在后续的迭代中，在正常情况下，代码会阻塞这里
			// 如果出现了rebalance：
			// 1. 心跳返回异常：有consumer新增或者退出
			// 2. partition变动
			// 都会导致cg.Next返回ErrGroupClosed，即说明要rebalance，进入一下个周期了
		...
		// 订阅，这里就会启动reader协程，不断的读取消息并放到kafka
		r.subscribe(gen.Assignments)

		// 启动offset提交协程，注意，启动该协程并非自动提交，这个协程只是监听一个channel并请求提交offset的api
		// 至于channel中的数据是哪里来的，就看提交方式了
		gen.Start(func(ctx context.Context) {
			r.commitLoop(ctx, gen)
		})

		// 启动ctx监听协程，ctx收到Done，说明generation旗下的协程有退出，就退订
		// Generation旗下的4个协程：
		// 1. 心跳协程
		// 2. Partition变动协程
		// 3. offset提交协程
		// 4. 退出订阅协程
		//
		// 其中退出订阅协程，就是本协程，就是当其他3个协程出现问题时，退订topic
		// 因为：
		// 1. 心跳收到异常回报：有消费者退出或者加入，rebalance
		// 2. Partition变动协程：Partition有加入或者宕机，rebalance
		// 3. offset提交协程：这个协程不会有退出（除非收到done）
		//
		// rebalance时，消费者就要停止消费，所以，这个版本的consumer，实现的是
		// Eager rebalance协议，而非Incremental Cooperative协议
		gen.Start(func(ctx context.Context) {
...
type ReaderConfig struct {
	...
	// 在创建reader的时候，如果传入的GroupID非空，就是创建kafka消费者组
	// 如果GroupID为空，就是独立消费者
	// 对于独立消费者，就要配置Partition，消费者组中的各消费成员，具体消费哪个Partition，由rebalance决定
	// 对于消费者组，不需要配置Partition
	GroupID string
	...
	// 对于消费者组，其各成员具体消费哪个Partition由rebalance机制决定，GroupBalancers配置项支持
	// 客户端传入自己的rebalance策略，所谓的rebalance策略，就是实现了GroupBalancer接口的类
	// 如果没有配置该配置项，那么会默认配置两种策略，range与roundbin
	...
	//
	// Only used when GroupID is set
	// 在消费者组的模式下，各个消费者成员必须要向coordinator定时发送心跳，已证明自己还活着，否则会被踢出
	// 组，引起消费者组中的成员变更，进而引起rebalance
...
// 创建消费者或消费者组的成员
// TODO问题：
// 1. 如果是创建消费者，那么消费者组是如何注册的
// 2. 调用NewReader创建消费者组的成员，就是新增消费者组的成员，如何进行rebalance的
func NewReader(config ReaderConfig) *Reader {
	...
	if config.GroupID != "" {
		// 当配置为消费者组的时候，如果没有配置balancer，那么就配置默认了range与roundRobin策略
...
func (r *Reader) ReadMessage(ctx context.Context) (Message, error) {
	...
	if r.useConsumerGroup() {
		// 这里可以发现，kafka-go没有具备配置auto_commit的能力，只要是使用了消费者组
		// 一律自动提交
...
func (r *Reader) FetchMessage(ctx context.Context) (Message, error) {
	...
	for {
		...
		// 惰性创建独立消费者的read携程
		// 当调用NewReader创建独立消费者后，是没有创建一个所谓的read携程去poll broker的消息的，此时，
		// closed == false，且由于是创建的独立消费者，所以version == 0
		// 当首次调用FetchMessage时，满足条件：!r.closed && r.version == 0，
		// 创建read协程，用于poll消息
		// 完成创建后，r.version++，所以，对于独立消费者，调用FetchMessage时，不会反复创建read协程
...
func (r *Reader) start(offsetsByPartition map[topicPartition]int64) {
	...
	// 这里并发的去创建各个Partition的read协程
	// 对于独立消费者，offsetsByPartition的长度一定是1，即一个独立消费者，总是消费一个Partition
	// 然而，对于消费组，offsetsByPartition的长度就不一定了，因为在消费者组，一个消费者成员，可以同时消费多个Partition
	// 例如，一个topic有3个Partition，一个成员数量为2的消费组去消费他，那么必然有一个成员需要消费2个Partition
	// TODO问题：
	// 有趣的是，并发创建read协程时，r.join，即waitGroup，并没有进行wait，这是为什么呢
...
// reader协程，也就是kafka java consumer里面，常见例子中的poll
// 这个协程几乎永不退出，除非ctx的cancel方法被调用
...
func (r *reader) run(ctx context.Context, offset int64) {
	...
	for attempt := 0; true; attempt++ {
		...
		if err != nil {
			if errors.Is(err, OffsetOutOfRange) {
				if r.offsetOutOfRangeError {
					// 这里有点奇怪，不是说该协程几乎不退出，除非ctx被cancel吗？
					// 首先，什么是OffsetOutOfRange错误？
					// 顾名思义，就是我们设置了一个consumer offset，但是，对于目标Partition里面来说，该offset越界了。
					// 例如：目标Partition的LEO为10，而消费者要拉去offset等于20的消息，就会报这个错误
					// 更常见的在于，如果Partition里面，没有消息数据，那么也会报这个错误
					// 这里是一个版本兼容，当读取到OffsetOutOfRange错误时，且配置中，配置了OffsetOutOfRangeError = true
					// 那么这里就会return，read协程退出
					r.sendError(ctx, err)
					return
				}
				...
				// 可以发现，如果越界了，那么就会continue，那么这里会不会导致一直continue呢？
				// 如果说，producer此时此刻，再也不会有消息produce，那么就会一直越界，一直continue
				// 然而这种情况，再也没有消息produce到kakfka，可能吗？如果可能的话，那还用什么kafka，都没生产者了
				// 只要有消息不断的produce，那么offset就会不断增加，那么终究会不在越界
				continue
			}
		...
		// timeout.
		// 代码走到了这里，说明read协程的初始成功了，因此attempt重置为0，为后续的poll操作做准备
		// attempt置零是有意义的，read协程在读取数据出错时，虽然用不退出，但是也要休眠一下，具体休眠多久，就是看attempt的次数
		// attempt的次数越多，休眠越久，具体算法见sleep方法，很简单
		attempt = 0
	...
	readLoop:
		for {
			...
			case errors.Is(err, UnknownTopicOrPartition):
				// 删除了topic或者Partition当然要报这个错误
				// 跳出readLoop，重新进行intialize
				r.withErrorLogger(func(log Logger) {
			...
			case errors.Is(err, OffsetOutOfRange):
				...
				default:
					...
					// 走到这里就是offset > last，即没有最新的消息生成了
					// 不会退出，继续read，直到last >= offset
					r.withErrorLogger(func(log Logger) {
...
// reader初始化:
// 1. 创建kafka链接
// 2. 返回消息offset，即reader从哪里开始消费，这也是在初始化时需要传入offset的原因
//
// 		创建kafka链接
// 		kafka的读与写，都是在Partition leader上进行，所以，reader首先就是创建与leader的链接
//
// 			****controller与metadata****
//
// 			那么这里存在一个问题：kafka集群中，有多个broker，那么哪个broker上存在reader需要的leader？
// 			难道要遍历所有的broker，尝试对其链接，然后找到目标broker？
// 			这显然是不可取的。
// 			kafka集群中，存在一个braker作为controller，该controller保存了kafka所有metadata，
// 			metadata就描述了Partition leader的具体位置。
// 			任何一个broker，在集群初始化时，都可能成为controller，那么如何选举，就是依托zookeeper进行抢占式选举，
// 			谁第一个在zookeeper中注册controller成功，谁就是controller
//			一个broker成为controller后，就会从zookeeper中同步metadata，并缓存到自己的内存中
//			然后广播给其他broker，其他broker将metadata进行缓存，所以所有broker都metadata
//
// 		因此，在创建leader链接时，并不需要遍历所有的broker，这也是在创建reader时，broker的配置只需几个broker地址，
//		并非所有的broker。然后，遍历向这些broker获取metadata，得到leader的目标broker进行连接即可
//		在进一步，既然理论上所有的broker都有metadata，为什么还要遍历呢？
//		因为controller广播metadata是异步的，是弱一致性的，因此存在数据不一致的可能性。试想一下：
//		当出现了leader重新选举后，leader发生了变化，在controller进行广播时，并不是所有的broker同时更新，总有些broker
//		因为各种各样的原因，出现了滞后，即本地维护的还是老的metadata。此刻，reader向这些‘滞后’的broker获取的metadata也是老旧的，
//		得到的leaderId是错误的，然后这个是链接该id的broker，就会得到 ‘Not Leader’的报错
//		所以，就需要遍历多个broker。
//
//		获得offset
//		初始化reader的第二步就是得到需要从哪个offset开始消费，消费者可以传入自己的offset，但是传入的offset不一定是有效的（kafka会定期删除掉过期数据）
//		例如：消费者希望存offset = 10开始消费，但是现在kafka的最小offset是20，那么offset将会被修正为20。
//		但是，如果kafka最大offset是10，消费者要求从20开始消费，就会报‘offset out of range’的报错
func (r *reader) initialize(ctx context.Context, offset int64) (conn *Conn, start int64, err error) {
	for i := 0; i != len(r.brokers) && conn == nil; i++ {
		...
		// seek到消费者期望的offset，如果offset越界，则报‘offset out of range’的报错
