...
// churn.
//
// Generation起到类似Erlang中supervisor的作用，当调用Start时，就类似调用了supervisor的
// spawn child方法。
// Generation Start方法期望spawn出来的协程是常驻的，一旦有些协程退出了，那么就通过context
// 通知Generation Start的其他协程进行退出，即一个协程死亡，其他协程跟着死。
// 当所有协程死亡后，consumer将进入next generation
...
func (cg *ConsumerGroup) run() {
	...
	var err error
	for {
		// nextGeneration会执行rebalance协议，消费者成员会会得到Partition的分配结果
		// 如果一切正常，即没有消费者的退出或者加入，没有Partition的退出或者新增，那么这里的调用
		// 会被阻塞，直到需要进行rebalance的时候
...
func (cg *ConsumerGroup) nextGeneration(memberID string) (string, error) {
	...
	//
	// TODO：
	// 本consumer group用的是以上哪个协议呢？
	//
	// 不管是哪种协议，第一步，就是找到coordinator。集群中任何broker都是coordinator，
	// 所有的broker在启动时，都会启动各自的coordinator，coordinator的主要职责就是消费者组的成员管理
	// 每个coordinator只负责‘属于’自己的消费者组，具体算法如下：
	// 1. 根据groupId的hash值与offset topic的分区数做余运算
	// 2. 得到一个分区号，找到该分区的leader所在broker，该broker的coordinator就负责该消费者组的管理
	...
	// rebalance协议的第二步就是join group
	// coordinator收到了‘join group’请求，说明有消费者加入到组中，说明要进行rebalance了，
	// 这时，coordinator就会把自己标记为rebalancing状态
	// 那么其他消费者如何感知到即将进行rebalance呢？
	// 注意，coordinator并不会主动向组中的所有消费者发起请求，告知他们即将rebalance，相反，
	// coordinator在将自己标记成rebalancing后，什么也不做。而各个消费者在向coordinator发起心跳时，
	// coordinator给心跳的回报会携带自己的状态信息，那么各个消费者在心跳回报中，就可以知道coordinator
	// 处于rebalancing状态，从而感知到即将发生rebalance
	//
	// 在向coordinator发生joinGroup请求后，由coordinator决定各个组员的Id，与群主的Id，并将这些
	// 信息response给消费者。该消费者得到自己的memberId与群主Id后，发现自己就是群主，于是就执行
	// Partition的分配
	// 所以joinGroup方法将返回一个groupAssignments，若该消费者不是群主，该返回值groupAssignments是空的结构体
	...
	// sync group
	// 协议的第三步，消费者向coordinator发送syncGroup消息，消费者中，所有的成员都会发送syncGroup请求
	// 群主的请求中带有分配结果的数据，而其他成员的请求参数中，该数据为空
	// syncGroup的返回值便是最终的分配结果
	// 也就是说其他非群主的消费成员，之所以可以通过向coordinator发起syncGroup请求，是因为群主在进行syncGroup时，
	// 携带了分配结果的信息，coordinator在得到该信息后，就可以通过syncGroup的response，返回给其他非群主的消费者，
	// 那么，如果其他非群主的消费者先于群主发起syncGroup请求会怎么样呢？
	// 在coordinator还没有收到群主的syncGroup时，会将其他的syncGroup请求挂起，即其他非群主成员阻塞，直到coordinator
	// 收到了群主的syncGroup后，会唤醒这些被挂起syncGroup，并将分配结果发回去。
	...
	// 知道了自己应该消费的Partition，就开始获取该Partition的offset
	...
	// 如果consumer group没有被主动关闭，那么代码将会被阻塞在这里，直到gen.done channel有信号传来
	select {
	case <-cg.done:
		gen.close()
		return memberID, ErrGroupClosed // ErrGroupClosed will trigger leave logic.
...
func (cg *ConsumerGroup) joinGroup(conn coordinator, memberID string) (string, int32, GroupMemberAssignments, error) {
	...
	// 向coordinator发起join group的请求
	// join group请求的response会包含该消费者的memberId与消费者群主leaderId等信息
	// 如果memberId == leaderId，就说明该消费者是群主
...
func (cg *ConsumerGroup) assignTopicPartitions(conn coordinator, group joinGroupResponseV1) (GroupMemberAssignments, error) {
	...
	// 执行分配，balancer可能是range balancer，也可能是round robin balancer，就看如何配置的了
